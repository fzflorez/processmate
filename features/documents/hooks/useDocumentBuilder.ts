...
class MockDocumentAIService {
      ...
      sections: [
        {
          id: generateUUID(),
          title: 'Introduction',
          content: 'This document was generated based on your specific requirements.',
          order: 1,
          type: 'paragraph',
        },
        {
        ...
        },
        {
          id: generateUUID(),
          title: 'Details',
          content: 'Detailed content would be generated here based on the AI analysis of your answers.',
          order: 3,
          type: 'paragraph',
        },
      ],
  ...
  async askDynamicQuestion(context: string, previousAnswers: DocumentAnswer[]): Promise<string> {
...
export function useDocumentBuilder({ userId, aiService = new MockDocumentAIService() }: UseDocumentBuilderOptions) {
  // State management
  ...
  // Answer management
  const updateAnswer = useCallback((questionId: string, value: DocumentAnswer['value']) => {
  ...
  const nextQuestion = useCallback(() => {
    ...
    setState(prev => {
      ...
      if (nextIndex >= state.selectedTemplate.questions.length) {
        return {
          ...prev,
  ...
  // Document export
  const exportDocument = useCallback(async (options: DocumentExportOptions) => {
